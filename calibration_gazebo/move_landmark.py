#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# based on https://github.com/oKermorgant/calibration_gazebo/blob/master/move_landmark.py
...
class Listener:
    ...
    def link_callback(self, msg):
        self.msg_ok = True
        # try to find a link called base_link or something
        for i,name in enumerate(msg.name):
            for words in name.split('/'):
                if 'base_link' in words:
                    # register this link as camera origin
                    self.cam_link = name
...
if __name__ == '__main__':
    ...
    if listener.cam_link != '':        
        rospy.loginfo('Found uavs\' base_link at %s, putting the board in front of a drone' % listener.cam_link)
    else:
        rospy.loginfo('Could not find uavs\' base_link, spawning landmark at (0,0,0.5)')
